...
class ASFF_4(nn.Module):
    def __init__(self, inter_dim=32):
...
class Downsample_x2(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.downsample = nn.Sequential(
            BasicConv(in_channels, out_channels, 2, 2)
        )
...
class Downsample_x4(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.downsample = nn.Sequential(
            BasicConv(in_channels, out_channels, 4, 4)
        )
...
class Downsample_x8(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.downsample = nn.Sequential(
            BasicConv(in_channels, out_channels, 8, 8)
        )
...
class HighResolutionModule_ASFF(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
                 num_channels, fuse_method, multi_scale_output=True):
        super(HighResolutionModule_ASFF, self).__init__()
        self._check_branches(
            num_branches, blocks, num_blocks, num_inchannels, num_channels)
        ...
        self.branches = self._make_branches(
            num_branches, blocks, num_blocks, num_channels)
        self.fuse_layers = self._make_fuse_layers()
        self.fuse_method =MWF([32,64,128,256])
        self.relu = nn.ReLU(True)
    ...
    def _make_ASFF_layers(self):
        ...
        for i in range(1):
            ...
            for j in range(num_branches):
                if j > i:
                    ASFF_layer.append(
                        ASFF_2(num_inchannels[i])
                    )
                elif j==i:
                    ASFF_layer.append(None)
                else:
                    ASFF_layer.append(
                        ASFF_2(num_inchannels[i])
                    )
            ...
            ASFF_layers.append(nn.ModuleList(ASFF_layer))
        return nn.ModuleList(ASFF_layers)
    def get_num_inchannels(self):
        return self.num_inchannels
    ...
    def forward(self, x):
        ...
        x_fuse = []
        x_fuse.append(self.fuse_method(x))
        # for i in range(len(self.fuse_layers)):
        #     y = x[0] if i == 0 else self.fuse_layers[i][0](x[0])
        #     for j in range(1, self.num_branches):
        #         if i == j:
        #             y = y+x[j]
        #         elif i<j and i==0:
        #             y = self.fuse_method[i][j](y , self.fuse_layers[i][j](x[j]) ) #对最后一层经过4个block的特征使低级特征上采样进行融合
        #         else:
        #             y = y +self.fuse_layers[i][j](x[j])
...
class BR_Pose(nn.Module):

    def __init__(self, cfg, **kwargs):
        self.inplanes = 64
        extra = cfg['MODEL']['EXTRA']
        super(BR_Pose, self).__init__()
        ...
        ###########BIFormerBlock########################################
        dp_rates=[x.item() for x in torch.linspace(0, 0., 6)]
        cur = 0
        topks=[1, 4, 16, -2]
        ...
        mlp_ratios=[4, 4, 4, 4]
        nheads= [dim // 32 for dim in qk_dims]
        self.biblock_stages = nn.ModuleList()
        for i in range(1):
            stage = nn.Sequential(
                *[Block(dim=pre_stage_channels[0], drop_path=dp_rates[cur + j],
                        ...
                        pre_norm=True,
                        auto_pad=False) for j in range(6)],
            )
            # if i in use_checkpoint_stages:
            #     stage = checkpoint_wrapper(stage)
            self.biblock_stages.append(stage)
            cur += 6
            ###########BIFormerBlock########################################
    ...
    def _make_stage(self, layer_config, num_inchannels,
        ...
        for i in range(num_modules):
            ...
            if num_modules==3 and i ==2:
                modules.append(HighResolutionModule_ASFF(
                    num_branches,
                    block,
                    num_blocks,
                    num_inchannels,
    ...
    def forward(self, x):
        ...
        y_list[0] = self.biblock_stages[0][:6](y_list[0])
